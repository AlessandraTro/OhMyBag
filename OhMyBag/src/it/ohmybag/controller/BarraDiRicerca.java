...
public class BarraDiRicerca extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// Recupera il campo di ricerca dal parametro della richiesta
		...
		if (searchField != null && !searchField.trim().isEmpty()) {
			...
			// Costruisce la risposta JSON con i prodotti corrispondenti
			StringBuilder json = new StringBuilder();
			json.append("[");
			for (Prodotto prodotto : matchProduct) {
				json.append("{").append("\"nome\":\"").append(prodotto.getNome()).append("\",").append("\"id\":\"")
						.append(prodotto.getId()).append("\"").append("},");
			}
