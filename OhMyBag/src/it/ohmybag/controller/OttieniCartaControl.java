    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (utente != null) {
            ...
            if ("checkCard".equals(action)) {
                ...
                try {
                    boolean cartaEsiste = cartaModel.checkCardExists(encryptedCardNumber + cardNumber.substring(15), utente.getUsername());
                    String jsonResponse = "{\"exists\": " + cartaEsiste + "}";
                    response.setContentType("application/json");
                    response.getWriter().write(jsonResponse);
                } catch (SQLException e) {
                    e.printStackTrace();
                    response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Errore durante la verifica della carta.");
                } catch (Exception e) {
            ...
            // Il resto del metodo doPost per gestire il salvataggio della carta
            try {
                ...
                if (selectedCardNumber != null && !selectedCardNumber.isEmpty()) {
                    ...
                    if (circuito != null && cardNumber != null && expiryMonth != null && expiryYear != null && cvc != null) {
                        String encryptedCardNumber = CriptoCard.encryptFirstNDigits(cardNumber, 15);
